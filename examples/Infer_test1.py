...
thin = 10
no_ch=3
adapt_lims = (2000,5000,5)
glob_lims = (2000,5000,5)
...
#run the MCMC with covariant steps, but now fix glob limits (by default 2.4^2/D)
Infer.MCMC(LogLikelihood_iid_mf,guess_pars[:],(Transit_aRs,time,flux),chain_len,err_pars,n_chains=no_ch,adapt_limits=adapt_lims,thin=thin)
#Get parameters values/errors from chains
par,par_err = Infer.AnalyseChains(conv/thin,n_chains=no_ch)
bf_par = Infer.GetBestFit(n_chains=no_ch)
print "Best Fit log p =", LogLikelihood_iid_mf(bf_par,Transit_aRs,time,flux)

#run and MCMC using the conditional dist as seeds
# guess_pars,err_pars = Infer.ConditionalErrors(LogLikelihood_iid_mf,guess_pars,err_pars,(Transit_aRs,time,flux),plot=0)
# adapt_lims = (0,0,0)
# Infer.MCMC(LogLikelihood_iid_mf,guess_pars[:],(Transit_aRs,time,flux),chain_len,err_pars,n_chains=no_ch,adapt_limits=adapt_lims,glob_limits=glob_lims,thin=thin)
# par,par_err = Infer.AnalyseChains(conv/thin,n_chains=no_ch)
